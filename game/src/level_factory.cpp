    ...
    for(auto it = objects->begin(); it < objects->end(); it++){
      ...
      if(type == Opts::HITBOX){
        ...
        mindscape_factory.fabricate_image(
          ...
          ASSETS_PATH(object["path"]), displacement, priority, dimensions_on_screen,
          dimensions_on_texture, coordinates_on_texture);

      }else if(type == Opts::TEXT){
        int priority = std::stoi(object["priority"]);
        std::string text = object["text"];
        std::string font_path = ASSETS_PATH(object["font_path"]);
        ...
        std::string path = ASSETS_PATH(object["path"]);
