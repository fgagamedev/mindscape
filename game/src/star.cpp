...
void Star::initialize_audio_effects(){
  engine::Audio * got_me_audio = new engine::Audio(
    "got_me",
    ASSETS_PATH("/audios/effects_songs/pegando_estrela.wav"),
    engine::Audio::CHUNK);
...
void Star::initialize_animations(){
  engine::Animation* star = create_animation(
    ASSETS_PATH("/images/sprites/items/star/star.png"),
    1, 6, 0.9, "LEFT"
  );

  engine::Animation* star_fading = create_animation(
    ASSETS_PATH("/images/sprites/items/star/star_caught.png"),
    1, 5, 0.5, "LEFT"
  );
